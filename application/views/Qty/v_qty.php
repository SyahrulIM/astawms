...
<div class="container-fluid">
    ...
    <style>
        ...
        /* Styling untuk produk tambahan */
        #additionalProductsTablePO {
            font-size: 0.85rem;
        }

        #additionalProductsTablePO th {
        ...
        #additionalProductsTablePO td {
            padding: 5px;
            vertical-align: middle;
        }
        
        #additionalProductsTablePO .form-control-sm,
        #additionalProductsTablePO .form-select-sm {
            font-size: 0.75rem;
            padding: 0.2rem 0.4rem;
            height: calc(1.5em + 0.4rem);
        }
        
        #additionalProductsTablePO .form-control-sm.price-additional {
            min-width: 90px;
        }
        
        #additionalProductsTablePO .form-control-sm.type-unit-additional {
            min-width: 70px;
        }
        
        #additionalProductsTablePO .form-control-sm.qty-additional {
            min-width: 80px;
        }
        
        #additionalProductsTablePO .description-additional {
            font-size: 0.75rem;
            padding: 0.2rem 0.4rem;
            height: auto;
            min-height: 60px;
        ...
        #additionalProductsTablePO .btn-sm {
            padding: 0.1rem 0.3rem;
            font-size: 0.7rem;
        }
        
        #additionalProductsTablePO small.text-muted {
            font-size: 0.7rem;
            display: block;
            margin-top: 2px;
        }

        /* Styling untuk pencarian produk */
        #productSearchResultsPO {
            background-color: #fff;
            border: 1px solid #dee2e6;
        }

        #productSearchResultsPO .table {
            margin-bottom: 0;
        }

        #productSearchResultsPO .table th {
        ...
        #productSearchResultsPO .product-item:hover {
            background-color: #f8f9fa;
            cursor: pointer;
        }
        ...
        #productSearchForm .input-group {
            margin-bottom: 10px;
        }

        #productSearchForm .form-text {
            font-size: 0.75rem;
            color: #6c757d;
        }
    </style>
...
<script>
    ...
    function initializePOProductAddition() {
        ...
        if (modalContent) {
            ...
            // Event untuk input di produk tambahan
            modalContent.addEventListener('input', function(e) {
                ...
                if (e.target.classList.contains('type-unit-additional')) {
                    updateAdditionalProductTypeUnit(e.target);
                }
                if (e.target.classList.contains('price-additional')) {
                    updateAdditionalProductPrice(e.target);
                }
                if (e.target.classList.contains('description-additional')) {
                    updateAdditionalProductDescription(e.target);
                }
            });
    ...
    function selectProductPO(button) {
        ...
        // Tambahkan ke array dengan default values
        additionalPOProducts.push({
            id: productId,
            sku: sku,
            nama: nama,
            type_sgs: '', // Default kosong
            type_unit: '', // Default kosong
            qty_order: 1, // Default 1
    ...
    function updateAdditionalProductsTable() {
        ...
        additionalPOProducts.forEach((product, index) => {
            ...
            // Row pertama: informasi produk dan form input
            const firstRow = document.createElement('tr');
            firstRow.setAttribute('data-index', index);
            firstRow.setAttribute('data-product-id', product.id);
            
            if (isLocalPO) {
                // Untuk local PO, qty_packing_list sama dengan qty_order
                firstRow.innerHTML = `
                    <td rowspan="2">${index + 1}</td>
                    <td rowspan="2"><strong>${product.sku || ''}</strong></td>
                    <td rowspan="2">${product.nama || ''}</td>
                    <td>
                    ...
                    <td>
                        <input type="text" 
                               class="form-control form-control-sm type-unit-additional" 
                               name="additional_products[${index}][type_unit]" 
                               value="${product.type_unit || ''}" 
                               placeholder="Unit"
                               data-index="${index}">
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="number" 
                               class="form-control form-control-sm price-additional" 
                               name="additional_products[${index}][price]" 
            ...
            secondRow.setAttribute('data-index', index);
            secondRow.setAttribute('data-product-id', product.id);
            secondRow.innerHTML = `
                <td colspan="6">
                    <div class="mb-1"><small><b>Description:</b></small></div>
                    <textarea class="form-control description-additional" 
                              name="additional_products[${index}][description]"
                              data-index="${index}"
                              rows="2"
        ...
        // Update header tabel untuk menampilkan kolom baru
        const tableHeader = document.querySelector('#additionalProductsTableContainer thead tr');
        if (tableHeader) {
            tableHeader.innerHTML = `
                <th width="4%">No</th>
                <th width="12%">SKU</th>
                <th width="18%">Nama Produk</th>
                <th width="12%">Type SGS</th>
                <th width="10%">Type Unit</th>
    ...
    // Update type_unit produk tambahan di array
    function updateAdditionalProductTypeUnit(input) {
        const index = parseInt(input.getAttribute('data-index'));
        if (isNaN(index) || index < 0 || index >= additionalPOProducts.length) {
            console.error('Invalid index:', index);
            return;
        ...
        const typeUnit = input.value.trim();
        
        if (additionalPOProducts[index]) {
            additionalPOProducts[index].type_unit = typeUnit;
        }
    }

    // Update price produk tambahan di array
    function updateAdditionalProductPrice(input) {
        const index = parseInt(input.getAttribute('data-index'));
        if (isNaN(index) || index < 0 || index >= additionalPOProducts.length) {
            console.error('Invalid index:', index);
            return;
        ...
        if (price < 0) {
            input.value = 0;
            showNotification('warning', 'Price tidak boleh negatif');
            return;
        }
        
        if (additionalPOProducts[index]) {
            additionalPOProducts[index].price = price;
        }
    }

    // Update description produk tambahan di array
    function updateAdditionalProductDescription(textarea) {
        ...
        if (additionalPOProducts[index]) {
            additionalPOProducts[index].description = description;
        }
    }
    ...
    function validateFormBeforeSubmit(event) {
        ...
        // Validasi price untuk produk tambahan (tidak boleh negatif)
        const additionalPriceInputs = document.querySelectorAll('.price-additional');
        ...
        if (hasInvalidPrice) {
            alert('Price untuk produk tambahan tidak boleh negatif.');
            event.preventDefault();
            return false;
        }
