            ...
            <div class="container-fluid">
                ...
                <div class="col">
                    <form action="<?= base_url('delivery_mutasi/exportExcel') ?>" method="post">
                        <div class="card mt-3">
                ...
                <div class="modal fade" id="addDeliver" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="addDeliverLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <form method="post" enctype="multipart/form-data" action="<?php echo base_url('Delivery_mutasi/createDelivery') ?>">
                ...
                <div class="modal fade" id="revisionDeliver" tabindex="-1" aria-labelledby="revisionDeliverLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <form method="post" enctype="multipart/form-data" action="<?= base_url('Delivery_mutasi/revisionDelivery') ?>">
            ...
            <script>
                $(document).ready(function() {
                    ...
                    function updateTableRow(id) {
                        $.ajax({
                            url: '<?= base_url("Delivery_mutasi/getDeliveryRow/") ?>' + id,
                            type: 'GET',
                            dataType: 'json',
            ...
            <div class="d-flex">
                ...
                <button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Close"></button>
                    ...
                    window.showConfirmModal = function(id) {
                        $('#confirmVerifikasiBtn').off('click').on('click', function() {
                            $.get('<?= site_url('delivery_mutasi/updateDelivery?id=') ?>' + id, function(response) {
                    ...
                    window.showValidasiModal = function(id) {
                        $('#confirmValidasiBtn').off('click').on('click', function() {
                            $.get('<?= site_url('delivery_mutasi/validasiDelivery?id=') ?>' + id, function(response) {
                                const result = typeof response === 'string' ? JSON.parse(response) : response;
                    ...
                    window.showFinalModal = function(id) {
                        $('#confirmFinalBtn').off('click').on('click', function() {
                            $.get('<?= site_url('delivery_mutasi/finalDelivery?id=') ?>' + id, function(response) {
                                const result = typeof response === 'string' ? JSON.parse(response) : response;
                ...
                function submitPhoto() {
                    ...
                    canvas.toBlob(function(blob) {
                        const file = new File([blob], "foto_mutasi.png", {
                            type: 'image/png'
                        });
                ...
                document.addEventListener('click', function(e) {
                    if (e.target.closest('.btn-delete')) {
                        ...
                        let deleteUrl = "<?= base_url('delivery_mutasi/deleteDelivery?id=') ?>" + id;
